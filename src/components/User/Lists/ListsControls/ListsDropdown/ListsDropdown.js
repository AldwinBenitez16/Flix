...
const listsDropdown = (props) => {
    let visible;
    let listsContent;
    if(props.accountLists) {
        visible = styles.Hide;
        if(props.show) {
            visible = styles.Show;
        }

        listsContent = <li onClick={props.toggleShowForm} className={styles.Center}>Create New List</li>
            ...
            listsContent = accountListsArray.map(list => {
                ...
                return ( 
                    <ListItem 
                        ...
                        showHandler={props.showHandler}>{name}</ListItem>
                );
            });
        };
    }
